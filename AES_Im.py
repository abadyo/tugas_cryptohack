    ...
    for i in range(len(list)):
        for j in range(len(list[i])):
            for k in range(len(list[i][j])):
                list[i][j][k] = int(hex(ord(list[i][j][k])), 16)
    return list
...
def matrix2sbox(list): 
    for i in range(len(list)):
        for j in range(len(list[i])):
            for k in range(len(list[i][j])):
                list[i][j][k] = lookup(list[i][j][k])
    return list

def matrix2sbox_r(list): 
    for i in range(len(list)):
        for j in range(len(list[i])):
            for k in range(len(list[i][j])):
                list[i][j][k] = hex(reverse_lookup(list[i][j][k]))
    return list
...
fin = open('image-PNG-icon.png', 'rb')
...
#     image_file.write(hasil)
    
fan = open('image.png', 'rb')
image = fan.read()
fan.close()
# print(image)

hasil_dk = dec('password12345677', image)
